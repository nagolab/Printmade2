...
// 라이센스 코드 // *.shinhan.com//var LICENSE_CODE = 'VHVZjlZTtvVv8NRDbVbzpRVpgX4h/fvvE/behPUm5jDwJ/fQCpPV9wpy6pFk+85mFaswJLQzK51xvHvdQ4YcVrFlFf5nV9hy8bq9AlFdb1w=';
var LICENSE_CODE = 'tkfWNR9KjoCpXbYmLdXr2JI001gvT81OU7OMSYqAYyf0y1lJD72BVlkVQ2U4GQKQ5onaRQTm2lNholWNpxmZPQ==';

var SERVER_PATH = '';
var FORMURL = '';
// ActiveX DLL 경로 - IE용 플러그인 업데이트를 위한 경로var OCX_CODEBASE = '';// Install 프로그램 경로들
var WIN_DOWNLOAD_URL = '';
var OCX_CODEBASE = '';
var MAC_DOWNLOAD_URL = '';
var UBUNTU_32BIT_URL = '';
var UBUNTU_64BIT_URL = '';
var FEDORA_32BIT_URL = '';
var FEDORA_64BIT_URL = '';

var domainDiv = document.location.href;
//운영과 개발을 나눈다.
if(domainDiv.indexOf("dev") >= 0){
	SERVER_PATH = 'http://devimg.shinhan.com/common/printmade2'; //개발	
	FORMURL = SERVER_PATH + '/server_files/form_dev.txt';
	WIN_DOWNLOAD_URL = SERVER_PATH + '/download_files/Windows/Printmade2_setup.exe';
}else{
	SERVER_PATH = 'http://img.shinhan.com/common/printmade2'; //운영	
	FORMURL = SERVER_PATH + '/server_files/form.txt';
	WIN_DOWNLOAD_URL = SERVER_PATH + '/download_files/Windows/Printmade2_setup.exe';
}
...
var PMTITLE = ' 신한은행'
//var PMLOGOIMG = SERVER_PATH + '/server_files/logo.png'; //Url
var PMLOGOIMG = ''; //Url	
var PMICON = SERVER_PATH + '/server_files/icon.png'; //Url
...
function PTMPrintWebPage(options)
{
	...
	if(options.frame_name != undefined && options.frame_name != ''){
		...
		var frameobject = Soo.Util.FindFrame(window.top, options.frame_name);
...
/* 		return; */
	...
	GPrintmade.option._title = PMTITLE;
 ...
 *
 **********************************************************************************************/
function PTMPrintWebPagePart(id){
	PTMPrintWebPage({frame_name:'main', elementid:'#'+id});
}

var isContentsChange = false;
//2012.12.03 외환센터 영역 안 나오는 현상 개선
function PTMPrintWebPageRibCon(){
	var height = 0;
	if (dmj('#con')[0]){
		isContentsChange = true;
		height = dmj('#con').height();

		dmj('#content_body').css('height', parseInt(height) + 'px');
	}

	PTMPrintWebPage({frame_name:'bizMain',elementid:'#contents'});
}

function PTMPrintWebPageCon(){
	var height = 0;
	if(dmj('#con')[0]){
		isContentsChange = true;
		height = dmj('#con').height();
...
function PTMPrintWebPageBankCon(){
	var height = 0;
	if(dmj('#con')[0]){
		isContentsChange = true;
		height = dmj('#con').height();
	...
	PTMPrintWebPage({frame_name:'bizMain',elementid:'#contents_bank'});
}

function PTMPrintWebPagePopup(id){
	if(dmj('#'+id).length == 0){
		alert(id+'를 찾을 수 없습니다.');
		return;
	}
	var $element = dmj('#'+id);
	var static_width = parseInt($element.width());
	$element.css('width', static_width+'px');
	PTMPrintWebPage({frame_name:'',elementid:'#'+id,objectcapture:false});
}

function PTMPrintWebPageWrapCon() {
	var $contents, $wrap, contents_height, wrap, wrap_height, wrap_scroll, wrap_overflow;

	$contents = dmj( '#contents' );
	$wrap = dmj( '#wrap' );

	if ( $contents.length > 0 && $wrap.length > 0 ) {

		contents_height = parseInt( $contents.css('height') );
		wrap_height = parseInt( $wrap.css('height') );

		wrap = document.getElementById('wrap');
		wrap_scroll = wrap.scrollTop;

		wrap_overflow = wrap.style.overflow;
		wrap.style.overflow = 'hidden';
		
		$wrap.css( 'height', (contents_height + 100) + 'px' );
	}

	PTMPrintWebPage({frame_name:''});

	if ( $contents.length > 0 && $wrap.length > 0 ) {
		setTimeout(function(){
			$wrap.css( 'height', wrap_height + 'px' );
			wrap.style.overflow = wrap_overflow;
			wrap.scrollTop = wrap_scroll;
		},3000);
	}
}
/**********************************************************************************************
 ...
 *
 **********************************************************************************************/
function PTM_BeforePrinting(options){
}
function PTM_AfterPrinting(options)
{
	if (isContentsChange) {
		dmj('#content_body').css('height', '');
	}
}
/**********************************************************************************************
 * 
...
var PTM_WorkAt = function(context){
	...
	/*
	dmj('a.locationHome, li, div, span',context).filter(function(){ 

		return this && dmj(this) && dmj(this).css('background-image') && (dmj(this).css('background-image').indexOf('http') != -1) }).each(function(){

		dmj(this).css('background-image',dmj(this).css('background-image'));

		dmj(this).attr('remove-background-image','1');
...
/* 		dmj(this).attr('selectedIndex',this.selectedIndex); */
	...
	dmj('body :checked',context).each(function(){
		dmj(this).attr('checked','checked');
	});
	
	//4. w2inputCalendar
	dmj('.w2inputCalendar',context).css('height',function(){
		return dmj(this).height();
	});
};
var PTM_AfterWorkingAt = function(context){
	//1. Restore background-image
//	alert('ContentBody Check');
	...
	/*
	dmj('body *[remove-background-image="1"]',context).each(function(){
...
*/
	...
	GetParameterString : function() {
		...
		this._AddParameter('TITLE', this._title);
...
var PTMInstaller = Soo.Class.create({
	...
	CheckVersion : function(plugin) {
		...
		try{
			if(GPrintmade.isWindows){
				current_viewer_version = new String(plugin.getFileVersion(appdatapath + '\\Printmade2\\Viewer.exe\0'));
			...
			this._Download(this._frame);
			var redirect_location = new String(location.href);
			var redirect_end  = redirect_location.indexOf('.com');
			if(redirect_end >= 0){
				redirect_location = redirect_location.substr(0,redirect_end+4); 
			}
			plugin.writeFile(redirect_location,plugin.getAppDataPath() + '/Printmade2/redirect.txt');
			return false;
		}
		if (this._CompareVersion(current_viewer_version, VIEWER_VERSION) == false)
		{
			alert('Printmade2 업데이트를 진행합니다.');
			this._Download(this._frame);
			var redirect_location = new String(location.href);
			var redirect_end  = redirect_location.indexOf('.com');
			if(redirect_end >= 0){
...
var PTMCaptureController = Soo.Class.create({
	__init__ : function(plugin,frame){
		...
		if(GPrintmade.isLinux){
			...
			if(appdatapath == '')
				this._imagefilepath = 'C:/PrintmadeTemp/img';
		}
...
//			left = left - 3;
		...
		var element_custom_style = ';;display:inline-block;position:'+$element.css('position')+';'+margin_top+margin_right+margin_left+margin_bottom+padding_top+padding_right+padding_left+padding_bottom+'width:' + width + 'px;height:' + height + 'px;background-position:0 0;background:url(\''+imagefilepath_background+'\');'+left_css+top_css;
		var element_class = '';
	...
	_InsertPlugin :function(){
		...
		for (var i = 0; i < 10000; i++) {
			...
			else
			{
				...
				if (this.isIE == true)  {
					dmj(this.target.document.body).append('<OBJECT id = "ptm_plugin" classid="clsid:8C4D012A-9B6C-4B87-A7CB-724BD7A9727C" codebase = "' + OCX_CODEBASE + '" width=1 height=1 align=center hspace=0 vspace=0 style = "display:none"></OBJECT>');
...
var PTMDOMParser = Soo.Class.create({
	...
	Run:function(target,plugincapture){
		...
		if(this._appdatapath  == '')
			this._appdatapath = 'C:/PrintmadeTemp';
...
//		}		
		...
		for(var i=0; i < tmp_frames.length; i++){
			...
			{
				tmp_frame_object.attr('src_local_ptm',dmj(tmp_frame_object).attr('src'));
				continue;
			}
		...
		try{
			if(GPrintmade.isLinux){
	            ...
	            if(GPrintmade.isSafari == true || GPrintmade.isSafariOver51 == true) {
					...
					if(this._appdatapath == 'C:/PrintmadeTemp') {
						this._plugin.writeFile(result, this._appdatapath + '/' + unique_id + '.html');
					} else{
		...
		if (unique_id == 1)
		{
			...
			if(GPrintmade.isMac){
				...
				if (GPrintmade.isIE == true)
				{
					...
					if(this._appdatapath == 'C:/PrintmadeTemp'){
						this._plugin.runViewer(path, '\"' + this._appdatapath + '/1.html\" \"' + GPrintmade.option.GetParameterString() + '\"',PMVIEWERMODAL);
					}
					else{
...
/* 			alert(this._plugin); */
	...
	_RemoveObjectTag : function (str, index, plugincapture) {
		...
		var regex = /<object([^>])*?\s+id\s*=\s*(\"|\')*(\S*)(\2)*[^>]*\>/i
	...
	_RelativeToStaticOfAddress : function(obj, currentURL){
		...
		dmj(obj).find('img').each(function(){
			if ((new String(dmj(this).attr('src'))).indexOf(current_protocol) == -1) {
		...
		// 모든 href 를 상대 경로에서 절대 경로로 바꾼다.
		dmj(obj).find('body').each(function(){
