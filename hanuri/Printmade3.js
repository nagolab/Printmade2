...
var PTMPathConverter = {
	...
	run : function(){
		...
		dmj("iframe",top.window.document).each(function(idx){
			dmj(this).attr("src_ptm",(idx+2)+".html");
			frames.push(this.contentWindow);
		});
		
		dmj("frame",top.window.document).each(function(){
			frames.push(this.contentWindow);
		});
	...
	checkInstall:function(){
			...
			try{
				if(this.checkWebBrowser() == "MSIE"){
					...
					return false;
				}else if(this.checkWebBrowser() == "Firefox"){
...
function PTM_LoadDialog(){
	 try{
	  ...
	  //var IMGURL = "http://192.168.1.39/4aphid/";
		...
		var message1 = "<img src='"+INSTALL_IMG_URL+"/message1.png' style='border:none'>";
		var message2 = "<img src='"+INSTALL_IMG_URL+"/message2.png' style='border:none'>";
		var message3 = "<img src='"+INSTALL_IMG_URL+"/message3.png' style='border:none'>";
		var divPopup = 	"<div id='ptm-loading' ptm-loading-remove='true' style='position:fixed;z-index:10001;top:"+download_div_top+"px;left:"+download_div_left+"px'>" +
			"<div id='PTMLoadImage' style='width:500px;height:310px;background-image:url(\""+INSTALL_IMG_URL+"printmade_loading.gif\");margin:0px auto;'>" +
				...
				"<div style='text-align:center;position:absolute;width:100%;top:90%;'>"+message1+"<a href='#' onclick='PTMLoadingClose()'>"+message2+"</a>"+message3+"</div>" +
			"</div>"+
		"</div>";
	  
	  /*
	  var divPopup =  "<div id='ptm-loading' ptm-loading-remove='true' style='position:fixed;z-index:10001;top:"+download_div_top+"px;left:"+download_div_left+"px'>" +
	       "<div id='PTMLoadImage' style='width:500px;height:310px;background-image:url(\""+INSTALL_IMG_URL
	       
	       +"printmade_loading.gif\");margin:0px auto;'>" +
	        "<div style='text-align:center;position:absolute;width:100%;top:70%;'><span></span></div>" +
	       // "<div style='text-align:center;position:absolute;width:100%;top:70%;'><span id='ptmdownuptext' style='font-size:12pt;font-weight: bold;'>?關? ?????????.</span></div>" +
	       //<span style='font-size:12pt;font-weight: bold;'>�ㅼ튂媛� �뺤긽�곸쑝濡� �대（�댁�吏� �딆븯�ㅻ㈃, <a id='ptmdownbutton' style='color:red;font-size:14pt;font-weight: bold;' href='#' onclick='PTMLoadingClose()'>�ш린</a>�먯꽌 �ㅼ튂�댁＜��떆��.</span>
	        "<div style='text-align:center;position:absolute;width:100%;top:90%;'></div>" +
	       "</div>"+
	      "</div>";*/
...
function PTMAntMessage(message){
	dmj("#PTMLoadImage > div > span").remove();
	dmj("#PTMLoadImage > div").append("<span>"+message+"</span>");
}
...
PTMAnt.Extend(PTMAnt,{
	init:function(options){
		this.isDataReceivedFromAphid = false;
		this.isPrintmadeRun = false;
		this.isClick = false;
		this.flag
	},
	runBridge : function(){
	...
	tryToConnect:function(){
		if(this.isClick == false){ // 클占쏙옙占쏙옙占쏙옙
			console.log("占쏙옙占쏙옙트占쏙옙占싱듸옙 占쏙옙占쏙옙")
			this.isClick = true;
		}
	...
	run:function(){
		...
		if(!this.isDataReceivedFromBridge){
			ptmcnt++;
			try{
				if(PTMChecker.checkWebBrowser() == "Chrome" && navigator.userAgent.match("OPR") == null){
					//if(localStorage.getItem('printmade3') != null){
				...
				console.log("占쏙옙占쏙옙트占쏙옙占싱듸옙 占쏙옙占쏙옙 占시듸옙 占쏙옙...");
			}
	...
	connectBridge:function(socket){
		try{
			...
			this.aphidConnector = new WebSocket(wsUri);
			this.aphidConnector.onopen = function(evt){ PTMAnt.aphidConnectorManager.onOpen(evt) };
			this.aphidConnector.onclose = function(evt){ PTMAnt.aphidConnectorManager.onClose(evt) };
			this.aphidConnector.onmessage = function(evt){ PTMAnt.aphidConnectorManager.onMessage(evt) };
			this.aphidConnector.onerror = function(evt){ PTMAnt.aphidConnectorManager.onError(evt) };
			this.isCreatedaphidConnector = true;
			this.Cnt++;
		}catch(e){
	...
	sendBridge:function(message){
		setTimeout(function(){
			try{
				PTMAnt.aphidConnector.send(message);
			}catch(e){
	...
	disconnectAphid:function(){
		this.aphidConnector.close();
	},
	getReceivedMessage:function(){
		return PTMAnt.aphidConnectorManager.getMessage();
	}
})

PTMAnt.Extend(PTMAnt.aphidConnectorManager,{
	onOpen : function(evt){
	...
	onMessage : function(evt){
		if((evt.data).search("Aphid Fine") > -1 || (evt.data).search("Bridge Fine") > -1){
			...
			console.log("APhid Version : " + PTMAnt.Version);
			if(PTMAnt.isDataReceivedFromAphid == false){
				setTimeout(function(){
					PTMPrintWebPageRun(PTMBridge.PrintmadeOption);
					clearInterval(PTMAnt.Ant);
					PTMAnt.isPrintmadeRun = true;},1000);
					PTMAnt.isDataReceivedFromAphid = true;
...
// ?湲�????? 
PTMBridge.init = function(option){
	// ????
...
PTMBridge.Run = function(option){
  if ( option == undefined )
		option = {};


  if(dmj("#printmadebutton").length > 0){
	dmj("#printmadebutton").css("display","none");
  }
	...
	var elementid = ""; // 1
	var ssloption = "U"; // 2
	var pagewidth = dmj('body')[0].scrollWidth; // 3
	
	
	var replaceAfter = ["!ptmprotocolhelper!","!ptms1!","print!ptmsn!made"];
	var replaceBefore = ["://","/","printmade"];
	
	var resultformurl = FORM_PATH;
	var resultscripturl = SCRIPT_PATH
	...
	var formurl = resultformurl; // 4
	var scripturl = resultscripturl; // 5
	
	
	
	var margin = "0,0,0,0"; // 6
	var align = "center"; // 7
	var framename = ""; // 8
	var targetpath = ""; // 9
	var pageheight = dmj('body')[0].scrollHeight; // 10
	var iframeid = "printMadeForm";
	

	if(this.Checker.checkInstall()){
		if(this.Browser == "MSIE"){
			...
			var pageWidth = dmj('body')[0].scrollWidth;
			if(option.elementid != undefined){
				...
				if(option.pagenumber != undefined){
					if(option.pagenumber == -1){
						margin = "10,5,5,5";
						formurl ="";
					}else if(option.pagenumber == 2){
						//formurl = "http!ptmprotocolhelper!192.168.1.105!ptms1!bms!ptms1!print!ptmsn!made2!ptms1!server_files!ptms1!form3.txt";
						formurl = resultformurl.replace("form.txt","form3.txt");
					}else{ // ???? pagenumber : 1

						  if(dmj("#printArea").length > 0){
							dmj("#printArea").css("height",document.body.scrollHeight);
						  }
						margin = "5,10,10,5";
						align = "left";
						//formurl = "http!ptmprotocolhelper!192.168.1.105!ptms1!bms!ptms1!print!ptmsn!made2!ptms1!server_files!ptms1!form.txt";
						formurl = resultformurl;
					}
				}else{
					margin = "0.1,0.1,0.1,0.1";
					align = "left";
					formurl ="";
					//formurl = "http!ptmprotocolhelper!192.168.1.105!ptms1!bms!ptms1!print!ptmsn!made2!ptms1!server_files!ptms1!form2.txt";
				}
			}
			
			
			var $targetIframe = dmj("iframe[name='printMadeForm']",top.window.document);
			if($targetIframe.length > 0){
				targetpath = $targetIframe.attr("src_ptm");
				var mydocument = $targetIframe[0].contentWindow.document;
			...
			myoption = myoption + makeParameter(option.elementid,ssloption,pagewidth,formurl,scripturl,margin,align,framename,targetpath,pageheight);
			location.href = myoption;

			  if(dmj("#printArea").length > 0){
				dmj("#printArea").css("height","");
			  }
  ...
  if(dmj("#printmadebutton").length > 0){
	dmj("#printmadebutton").css("display","");
  }
...
function settingURL(){
	...
	if(location.port != ""){
		SERVER_PATH = location.protocol + '//' + location.hostname + ':' + location.port+ '/bms/printmade2/';
		SCRIPT_PATH = SERVER_PATH + "server_files/scripts.js";
		...
		INSTALL_IMG_URL = location.protocol + '//' + location.hostname + ":" + location.port+ '/bms/printmade2/server_files/imgs/';
		FORM_PATH = location.protocol + '//' + location.hostname + ":" + location.port + '/bms/printmade2/server_files/form.txt';
		INSTALL_DIV_MSG = '<img src="'+INSTALL_IMG_URL+'setup_text_korea.png"/>';
	}else{
		SERVER_PATH = location.protocol + '//' + location.hostname + '/bms/printmade2/';
		SCRIPT_PATH = SERVER_PATH + "server_files/scripts.js";
		WIN_DOWNLOAD_URL = SERVER_PATH + '/download_files/Windows/Printmade3_setup.exe';
		MAC_DOWNLOAD_URL = SERVER_PATH + '/download_files/MacOSX/Printmade3forMac.dmg'
		UBUNTU_32BIT_URL = SERVER_PATH + '/download_files/Linux/Printmade3_Ubuntu_32bit.tar';
		...
		FEDORA_32BIT_URL = SERVER_PATH + '/download_files/Linux/Printmade3_Fedora_32bit.tar';
		FEDORA_64BIT_URL = SERVER_PATH + '/download_files/Linux/Printmade3_Fedora_64bit.tar';
		INSTALL_IMG_URL = location.protocol + '//' + location.hostname + '/bms/printmade2/server_files/imgs/';
		FORM_PATH = location.protocol + '//' + location.hostname + '/bms/printmade2/server_files/form.txt';

	}

}

function PTMPrintWebPage(option){
	settingURL();
	PTMBridge.init(option);
	PTMBridge.Run(option);
}
 ...
 *
 *								
 *							Printmade2 JavaScript(2.5.0.0) with Aphid (Non Plugin)
 *
 * 
 *
 ...
 * Tel : +82 2 3472 1515
 * Copyright 2000 ~ 2015 Nagosoft Co., Ltd. All Right Reserved.
 *  
 **********************************************************************************************/
...
// ??????? ???
//
var LICENSE_CODE = 'suwNYZ2/gGOQhiBq4rMpwkn5BpJKS1quBol5x54ayoqbBfPLNqrHJ9RwzXAeEXesVSfjid4oT+G8BR0AYtG6gw==';
// ??????? ???????
...
// ???? ???
var SERVER_PATH = location.protocol + '//' + location.hostname + '/bms/printmade2';
// ActiveX DLL ??? - IE?? ?첨????? ????????? ???? ???
...
// ?????? ????(Margin) - '??,??,??,??'
var PAGEMARGIN = '0.1,0.1,0.1,0.1';
...
// DownloadDiv ???????. NonCrossTop ??? ????(body?? ??? top?? a???????)
 ...
 * 
 **********************************************************************************************/
var VALIDATECLASS_MSG = 'JavaScript?? ????? ?琯???? ???????.\n';
var UNDEFINED_WEBBROWSER_MSG = '???? ?????? ??? ????????? ???????? ??????.';
var UNDEFINED_OS_MSG = '???? ?????? ??? OS?? ???????? ??????.';
//var INSTALL_DIV_MSG = '?? ???慣???? ?? ?????? ?關? ???? ????????.';
var INSTALL_DIV_MSG = '<img src="'+INSTALL_IMG_URL+'setup_text_korea.png"/>';
var INSTALL_LINUX_MSG = '???? ?????? ???? ??, install.sh?? ???恝??? ???????y?'; //??????? ?????? ??????? ????.

var UPDATE_ERROR_MSG = 'Printmade 2 ??????? ???????? ???? ???????. ?? ?????? ???????.';
 ...
 *
 *  - Parameters -
 * frame_name : ????? ???????? name ??? id?? ??????. ??u ?????? 캡????? ?? ????? '' ?? ??쨈?.
 * elementid : 캡??????? Element?? id?? ??쨈?. CSS Selector o?? #[id] ???쨌? ??쨈?.
 * objectcapture : ????? 캡???? ??????罐? ???????. ?첨??챨? ?????? ?? ??? ???????. 0?? ???????, 1?? ?????.(true,false)
 * cutrect : 캡??? ??????? ?關??? ?? '??,??,??,??' ?? ??? ??? ??????? ???????.
 * formvar : ???????? ?????? ????? ?????? ???? ???? ??쨈?.
 * formurl : ????????? ??罐? ???????. ???????? ?????? FORMURL ????? ??????.
 *
 * 12.03.26 frame_index ???
 * frame_index : '$1.2' -> frames[1].frames[2] a??
 *
 * 12.04.09 fullcapture ???
 * fullcapture : true ???? ?? 캡?? ????(fullcapture ???? ?? objectcapture?? ??????? true?? ????)
 *
 * 12.12.27
 * renderingtype, orientation, alignment ???
 *
 * PTMPrintWebPageRun
 
 ({frame_name:'',cutrect:'0,0,0,0'});
 *
 **********************************************************************************************/
 var ptmcnt = 0;
...
{
ptmcnt = 0;
	// ptmcnt > 2 && ptmstarted == false
  // PTMPrintWebPageRun() ??????? ????? ??? options ???? undefined?? ???
  // ????? options.frame_name???? ??????.
	/*
	if($targetIframe.length > 0){
		targetpath = $targetIframe.attr("src_ptm");
		var mydocument = $targetIframe[0].contentWindow.document;
		PMPAGEWIDTH = dmj("body",mydocument)[0].scrollWidth;
		//pageheight = dmj("body",mydocument)[0].scrollHeight;
	}else{
		targetpath = "1.html";
	}*/
  ...
  if ( options == undefined )
	...
	var $targetIframe = dmj("iframe[name='printMadeForm']",top.window.document);
	if($targetIframe.length > 0){
		dmj("iframe[name='printMadeForm']",top.window.document).attr('name','printMadeForm');
		options.frame_name = "printMadeForm";
	}
	 ...
	 * 12.03.26 
	 * frame_index ?????? ???
	 * frame_element : '$0.1' ???쨌?, NonCrossTop ???? ??? ???
	 */
	if ( options.frame_index != undefined ) {
		...
		if ( typeof frameobject == 'string') {
			alert('PTMPrintWebPage ????? [frame_index]?? ?u?? ???????? ??????? ???? ??????.\n???恝??? : ' + frameobject);
			return;
		} else {
			targetframe = frameobject;
	...
	if ( dmj('frameset',targetframe.document)[0] != undefined ) {
		alert('PTMPrintWebPage ????? [frame_name]?? frame?? frameset?? ??? body?? ?????? ???? ????.');
 		return;
	 ...
	 * PTM_BeforePrinting
	 *
	 * ?關??? ???? ????? ??????? PTM_BeforePrinting ??????? ??????.
	 * 12.03.27 ??? ???? : targetframe?? ???? ???쩔????? ?關? ?? ????? ??? ???? 
	 * ??? ??? targetframe?? ???? ???? ?觀??? ????? ????.
	 */ 
	...
	if ( options.elementid != undefined ) {
		if ( dmj(options.elementid, targetframe.document).length == 0 ) {
			alert( 'PTMPrintWebPage ????? elementid?? Element?? a?? ?? ??????.' );
			return;
		}
	...
	GPrintmade.option.SetPageHeight( PMPAGEHEIGHT );     
...
function PTM_BeforePrinting( options, targetframe ) {
	...
	if($targetIframe.length > 0){
		PMPAGEWIDTH = '';
		//PMPAGEHEIGHT = dmj("body",mydocument)[0].scrollHeight;
	}
...
function PTM_DownloadDialog() {
	...
	dmj("div",frame.document).each(function(){  // div???? OVERFLOW css ??????? "scroll"??? "scroll-y"?? ???? ???
	...
	if (GPrintmade.isWindows == true) {
		...
		$down_div.append('<a href = "' + download_url + '" id = "ptm_download" ><img alt = "???琯?" src = "' + INSTALL_IMG_URL + 'download.png" style = "border:0"></a>');			
		...
		$down_div.append('<a href = "' + fedora_url + '" id = "ptm_fedora_download" '+installmessage+'><img alt = "???琯?" src = "' + INSTALL_IMG_URL + 'download_fedora.png" style = "border:0"></a>&nbsp;');
		$down_div.append('<a href = "' + ubuntu_url + '" id = "ptm_ubuntu_download" '+installmessage+'><img alt = "???琯?" src = "' + INSTALL_IMG_URL + 'download_ubuntu.png" style = "border:0"></a>&nbsp;');		                
	}
	...
	$down_div.append( '<a href = "#" id = "ptm_cancel"><img alt = "???" src = "' + INSTALL_IMG_URL + 'cancel.png" style = "border:0"></a>' );
...
var PTMOptionObject = Soo.Class.create({
	...
	GetParameterString : function() {
		...
		this._AddParameter('PAGEHEIGHT',this._pageheight);
...
var PTMCaptureController = Soo.Class.create({
	__init__ : function ( plugin, frame ) {
		...
		if ( GPrintmade.isLinux ) {
			this._imagefilepath = this._plugin.getTempPath() + "/Printmade3/temp/img";
		} else {
			var appdatapath = this._plugin.getAppDataPath();
			this._imagefilepath = appdatapath.replace( /\\/g, '/' );
			this._imagefilepath = this._imagefilepath + "/Printmade3/temp/img";
		}
	...
	_GetElementForFrame : function ( frame ) {
		/*
		 * frame : a???? ??? ???????? DOMWindow ???????.
		 * frame?? ?????? HTMLIFrameElement ??? HTMLFrameElement?? a????? ???.
		 */
		var
			parent, frm, ifrm;
...
var PTMCaptureEngine = Soo.Class.create({
	__init__:function( plugin, frame, imagefilepath ) {
		...
		//Linux - Firefox???? document.documentElement?? scrollWidth?? ???????.
...
//		this._body_height = TBody.scrollHeight;//document.body.scrollHeight;
	...
	Calculate : function ( plugincapture ) {
		...
		/*
		 * Body?? height?? width?? window?? innerHeight, innerWidth?? ????? ?? ???? 캡?캡? ?? ?? ???? ??????.
		 * i => y
		 * j => x
		 */
...
*/
		...
		/*
		 * Body Height?? Window innerHeight?? ????? ???管? ?????? ??????? ?? ??????? ????.
		 */
	...
	_Move:function(){
		...
		else if(GPrintmade.isOpera){   // 5.17 ??? ?觀?. Opera?????? document.body.scrollTop ???????? ????.
...
var PTMMainController = Soo.Class.create({
	...
	Run:function(target){
		...
		if(GPrintmade.isFirefoxOver3 == true || GPrintmade.isFirefox3 == true){
		...
		switch (browser){
			...
			case 'Netscape':
				if (useragent.lastIndexOf('Chrome/') > 0) {
					...
					// Maxthon ???????? ??荑�?? Object-Embed ??첩?? ??? Embed ???? SafariOver51?? ???? o??
...
/* 		this._RemovePrintmade2(); */
	...
	/*
	 * Frame ???? ????? ???
	 * ?? ???? - ?? ???????? window ??????? root ???, head, body(or frameset)?? ???? ???
	 */
	_ParseFrame:function(framewin,unique_id){
		var head_object;

		// head?? ???? framewin.document ?? ?????????? ???.
		head_object = dmj('head',framewin.document);

		//PTMFrameObj ?? framewin?? body ??? frameset ?? ?????? ???????? ???? ?????
		...
		for(var i=0; i < tmp_frames.length; i++){
			...
			//?? frame?? Attribute????? frame_unique_id?? ???灌?. ????? ??????? ????? 
		...
		//img ??? png, jpg, jpeg, gif, tif,  tiff, bmp ?? ??? ??? 캡?캡? ???.
	...
	_FindPagebreakTag : function (frame) {
		...
		if (pagebreaks.length == 0) {
			...
			for (var i = 0; i < pagebreaks.length; i++) {
				...
				// IE8.0 MS ???? ???????? ??? tmpelement?? offsetParent?? a????? BODY?? ????
...
/* 		this._insertID(framewin); */
		...
		result = result + '<head><script src="'+SCRIPT_PATH+'"></script><script>window.onload = function(){PTM_RemoveDialog()}</script>' + result_head + '</head>';
...
//	result = result + this._ResizeBody();
	...
	_RemoveObjectTag : function (str, index, plugincapture) {
		...
		if (openindex != -1 && openindex < closeindex) {	
			 ...
			 * openindex : ???? ????? ???? ???? ?????? <object> ?짹??? ???
			 * closeindex : ???? ????? ???? ???? ?????? </object> ?짹??? ???
			 * openindex?? ?? ???? ???? ?????? object ?짹?? ?????? ?? ???恝? object ?짹?? ?????? ???? ??????.
			 * ?????? 2???? </object>?? ??????? ???.
			 * closeindex?? ?? ???? ???? </object> ?짹?? ???? ???쨈?? ?????
			 * openindex?? -1?? ???? ?? ??? ???恝? object ?짹?? ?????? ??쨈?? ????? ??????
			 * ?????? object ?짹?? ???? ???? ?????? </object> ?짹?? ???????? ???.
			 */
	...
	__ConvertAddress:function(obj,tagName,attrName,CattrName,static_path,current_protocol){
		dmj(obj).find(tagName).each(function(){
//			if ( dmj(this).attr( 'src' ) == undefined) {
//				return;
//			}

		  /*
		   * ??????? ?????????? ???? ???????? ??? + http?????? ???(??. https ???????? img src ????? http ???????? url?? ???)
		   */
		   if(attrName == 'src'){
				var attributeName = dmj(this).attr(attrName);
				if ((new String(attributeName)).indexOf(current_protocol) == -1 && 
					(new String(attributeName)).indexOf('http://') == -1 && 
					(new String(attributeName)).indexOf('https://') == -1  ) {
					
					var src_string = new String(this.src);
					if(src_string.indexOf(current_protocol) == -1){
						if(src_string.indexOf('/') == 0){
							//Root relative path
							dmj(this).attr(CattrName, current_protocol + '//' + document.location.host+'/' + (new String(attributeName)));	
						}else{
							//Document relative path
							dmj(this).attr(CattrName, static_path + (new String(attributeName)));
						}
					} else {
						//absolute path?? ?????? ??? ???.
						dmj(this).attr(CattrName,src_string);
					}
				}
			}else{
				var attributeName = dmj(this).attr(attrName);
				if ((new String(attributeName)).indexOf(current_protocol) == -1 && 
					...
					if(href_string.indexOf(current_protocol) == -1 && href_string.indexOf('javascript') == -1){
						...
						//absolute path?? ?????? ??? ???.
						dmj(this).attr(CattrName,href_string);
					}
				}else{
					dmj(this).attr(CattrName,dmj(this).attr('href'));
				}
			}
		});
	},
     ...
     */
		...
		//SRC
		this.__ConvertAddress(obj,'img','src','src_ptm',static_path,current_protocol);
		this.__ConvertAddress(obj,'input','src','src_ptm',static_path,current_protocol);
		this.__ConvertAddress(obj,'link','href','href_ptm',static_path,current_protocol);
		this.__ConvertAddress(obj,'*[background]','href','href_ptm',static_path,current_protocol);
