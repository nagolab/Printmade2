 ...
 *																	 ____ 
 *				  ____       _       _                       _      |__  \   
 *				 |  _ \ _ __(_)_ __ | |_ _ __ ___   __ _  __| | ___  __) | 
 *				 | |_) | '__| | '_ \| __| '_ ` _ \ / _` |/ _` |/ _ \|__  /  
 *				 |  __/| |  | | | | | |_| | | | | | (_| | (_| |  __/ __) |   
 *				 |_|   |_|  |_|_| |_|\__|_| |_| |_|\__,_|\__,_|\___||____/																														
 *
 *								
 *							Printmade3 JavaScript
 *
 * 
 *
 ...
 *			 Linux - Ubuntu 32bit, Ubuntu 64bit, Fedora 32bit, Fedora 64bit
 * 					 (Firefox, Chrome)
 *
...
// 라이센스 코드
var LICENSE_CODE = '/1gwQZl/A7qcodaYQTl3tAdZeoz3ELR4kD8+VM8+0P6uN8yedxyL9f6d3nl2fKP2srf55/r6DDtpB/faLMMogi79o+NOu6o8+7CXWyVJ5URxGwDakpHHs5slOHPSKd63k7kJyBOCXVsfvMBJIsWiKgoeZLAqC+nra2XJLiiH94Y=';
...
// 공통 경로
var SERVER_PATH = location.protocol + '//' + location.hostname + '/sw/printmade';
...
// 양식파일 경로
var FORMURL = SERVER_PATH + '/server_files/form/form.txt';
...
function PTMLoadingClose(){
	...
	location.href="/sw/wizvera/veraport/install20/install.html";
	//PTM_DownloadDialog();
}
...
var shinhan = {
	fixForm : {}, // 신한은행에 맞게 양식 URL을 받아온다 (반환형 String)
	getElementIdInXplatform : {}, // Xplatform에서 받아온 ID 값을 문서에 유효하게 변환한다 (반환형 String)
	removeOnloadAttr:{}

}

shinhan.fixForm = function(){
	//var formRoot = 'https://' + location.hostname + '/sw/printmade/server_files/';
    var formRoot = 'https://bizbank.shinhan.com/sw/printmade/server_files/'; // 수정이 필요합니다.
	if(location.hostname.match("biztst") != null){ // 이관
		LICENSE_CODE = 'qikiyQExskBUwiQxcTQIoIVMBcwK4cd4cbKVq73NspVVJ+OJ3ihP4bwFHQBi0bqD'; // 개발용(1년)
		return formRoot+"formTST.txt";
	}else if(location.hostname.match("bizdev") != null){
		LICENSE_CODE = 'qikiyQExskBUwiQxcTQIoIVMBcwK4cd4cbKVq73NspVVJ+OJ3ihP4bwFHQBi0bqD'; // 개발용(1년)
		return formRoot+"formDEV.txt";
	}else{
		LICENSE_CODE = '/1gwQZl/A7qcodaYQTl3tAdZeoz3ELR4kD8+VM8+0P6uN8yedxyL9f6d3nl2fKP2srf55/r6DDtpB/faLMMogi79o+NOu6o8+7CXWyVJ5URxGwDakpHHs5slOHPSKd63k7kJyBOCXVsfvMBJIsWiKgoeZLAqC+nra2XJLiiH94Y=';
		return formRoot+"form.txt";
	}
} 

shinhan.getElementIdInXplatform = function(eid,targetframe){
	var targetid = eid.replace("#","");
	var regex = new RegExp(targetid,"i");
	var elementid = dmj('div',targetframe.document).filter(function() {return this.id.match(regex);}).eq(0).attr("id");

	if(elementid == undefined){
		if(targetid.match("mainframe_frameset_childframe_") != null){
			targetid = targetid.replace("mainframe_frameset_childframe_","");
			elementid = dmj('div[id*="'+targetid+'"]',targetframe.document).filter(function(){return dmj(this).attr("id").match("div_Work_tabMain") == null}).eq(0).attr("id");
		}
		if(targetid.match("mainframe_frameset_childframecontents_") != null){
			targetid = targetid.replace("mainframe_frameset_childframecontents_","");
			elementid = dmj('div[id*="'+targetid+'"]',targetframe.document).filter(function(){return dmj(this).attr("id").match("div_Work_tabMain") == null}).eq(0).attr("id");
		}
	}

	if(elementid != "undefined" && elementid != "" && elementid != undefined && dmj("#"+elementid,targetframe.document).length > 0){
		return "#"+elementid;
	}else{
		return "";
	}
...
shinhan.removeOnloadAttr = function(){

	dmj("*[onload]",top.window.document).each(function(){
	  dmj(this).removeAttr("onload");  
	});
	
	dmj("*[onload]").each(function(){
	  dmj(this).removeAttr("onload");  
	});
...
{
  // PTMPrintWebPageRun() 형식으로 호출할 경우 options 값이 undefined가 되어
  // 아래의 options.frame_name에서 에러남.
	...
	// 1. 신한은행 양식 관리
	options.formurl = shinhan.fixForm();
	console.log(options.formurl);
...
function PTM_BeforePrinting( options, targetframe ) {
	...
	if(dmj(options.elementid).length == 0 && options.elementid != undefined){
		options.elementid = shinhan.getElementIdInXplatform(options.elementid,targetframe);
	}
	shinhan.removeOnloadAttr();
	
	
	if(options.isPopup == 1){
	  GPTMPOPUPID = options.elementid;
	  PMPAGEWIDTH = "";
	  if(GPTMPOPUPID == ""){
	    alert("ERROR : 팝업인쇄영역을 지정하지 않았습니다. 관리자에게 문의해주시기 바랍니다.");
	  }
	}else if(options.elementid != "" && options.elementid != undefined){
 		PMPAGEWIDTH = 980;
		GPTMELEMENTID = options.elementid;
		if(GPTMELEMENTID == ""){
			alert("ERROR : 인쇄영역을 지정하지 않았습니다. 관리자에게 문의해주시기 바랍니다.");
		}
	}else{
 		PMPAGEWIDTH = 980;	  
	}
...
function PTM_DownloadDialog() {
	...
	dmj("body").append( '<div id = "ptm_down" align = center style="position:absolute;top:' + download_div_top + 'px;left:' + download_div_left + 'px;width:'+dialog_width+'px;height:'+dialog_height+'px;background-image:url(\'' + INSTALL_IMG_URL + 'setup.png\');z-index:5001;display:block"></div>' );
...
/* 		this._insertID(framewin); */
		...
		// 신한수정 4
		var nanumfont = 'https://'+location.host+'/css/nanumgothic.css'; // 20150211
		result_head = result_head.replace(nanumfont,"");

		nanumfont = 'https://'+location.host+'/css/nanumgothic_dev.css';
		result_head = result_head.replace(nanumfont,"");

		nanumfont = 'https://'+location.host+'/css/nanumgothic_tst.css';
		result_head = result_head.replace(nanumfont,"");
		//
		...
		/* 신한수정 5 */
		if(unique_id==1){
			result = result + '<head><script>'+
			' var PTMPOPUPID="'+GPTMPOPUPID+'";'+
			' var PTMELEMENTID="'+GPTMELEMENTID+'";'+
			'</script>'+
			'<script src="'+SCRIPTURL+'"></script>' + result_head + '</head>'; // 내장스크립트
		...
		try{
    		...
    		dmj("*[id][style*='border-image-source']",top.window.document).each(function(){
    		  var borderURL = dmj(this).css("border-image-source");
              var borderWidth = dmj(this).css("border-width");
              borderWidth = borderWidth.replace(/px/gi,"");
              var borderImageStyle = borderURL + " " + borderWidth + " stretch";
              dmj(this).attr("ptmBorderStyle",borderImageStyle);
        })
        
        	dmj("*[id][style*='border-image']",top.window.document).each(function(){
    		  var borderURL = dmj(this).css("border-image-source");
              var borderWidth = dmj(this).css("border-image-slice");
              borderWidth = borderWidth.replace(/px/gi,"");
              var borderImageStyle = borderURL + " " + borderWidth + " stretch";
              dmj(this).attr("ptmBorderStyle2",borderImageStyle);
        })
