 ...
 *							Printmade2 JavaScript Standard Version (2.0.1.0.3)
 *	
 * 
 *
 ...
 * Tel : +82 2 3472 1515
 * Copyright 2000 ~ 2013 Nagosoft Co., Ltd. All Right Reserved.
 *  
 **********************************************************************************************/
...
var PTMBridge;
//var SERVER_PATH = 'https://open.ibk.co.kr/ois/download/printmade3';


// 라이센스 코드

//var LICENSE_CODE = '';
var LICENSE_CODE = 'VVm4zvxbyNDKK+xVf0TDOKstYbIbB6J/KX7NRkfkfvDmidpFBObaU2GiVY2nGZk9';
...
function ispopupPrint(){
	return (dmj("#printmade_area").length > 0 && dmj("#printmade_area").height() > 0 && dmj('#ibkpop_contents').length > 0);
}
		...
		Run : function(option){
			...
			if(this.checkInstall()){
				...
				/*if(this.Browser == "MSIE"){
					this.Converter.run();		
					dmj("#PTMButton",this.targetframe.document).css("display","none");
					//alert("printmade://I|container|U|"+dmj('body')[0].scrollWidth+"|http!ptmprotocolhelper!www.nagosoft.com!ptms1!print!ptmsn!made2!ptms1!server_files!ptms1!form.txt");
					
					if(option.elementid != undefined){
						elementid = option.elementid;
					}
					if(iframeid != ""){
						targetpath = dmj("#"+iframeid).attr("src_ptm");
						var mydocument = dmj("#"+iframeid)[0].contentWindow.document;
						pagewidth = dmj("body",mydocument)[0].scrollWidth;
						pageheight = dmj("body",mydocument)[0].scrollHeight;
					}else{
						targetpath = "1.html";
					}
					myoption = myoption + makeParameter(elementid,ssloption,pagewidth,formurl,scripturl,margin,align,framename,targetpath,pageheight);
					
					
					location.href = myoption;
					
					dmj("#PTMButton").css("display","block");
				}else*/
				if(this.Browser == "Firefox" || this.Browser == "MSIE"){
		...
		checkInstall : function(){
			...
			try{
				if(PTMChecker.checkWebBrowser() == "MSIE"){
					...
					//	if(PTMCheckVersion.check([ptmversion[ptmversion.length-1].replace("printmade=","")],[VIEWER_VERSION])){
					//		return true;
					//	}else{
					//		return false;
					//	}
...
var PTMCheckVersion = {
	...
	convertDigitVersion: function(versionData){
		var version = versionData[0].split(".");
		for(i=0;i<4;i++){
...
function PTM_LoadDialog(){
	try{
		...
		var clientWidth = dmj(window).width();
		var clientHeight = dmj(window).height();
		var max = 0;
		var maxScrollHeight = 0;
		...
		var download_div_left =  (clientWidth - dialog_width) / 2 ;//scrollLeft + (clientWidth - dialog_width) / 2 ;
		var download_div_top = (clientHeight - dialog_height) / 2 ;//scrollTop + (clientHeight - dialog_height) / 2 ;
		...
		var divPopup = 	"<div id='ptm-loading' ptm-loading-remove='true' style='position:fixed;z-index:2000002;top:"+download_div_top+"px;left:"+download_div_left+"px'>" +
							"<div id='PTMLoadImage' style='width:500px;height:310px;background-image:url(\""+INSTALL_IMG_URL+"printmade_loading.gif\");margin:0px auto;'>" +
								...
								"<div style='text-align:center;position:absolute;width:100%;top:90%;'><span>프린트메이드가 설치되지 않았을 경우, <a href='#' onclick='PTMLoadingClose()'>아래</a>를 눌러주십시오.</span></div>" +
							"</div>"+
						"</div>";
...
PTMAnt.Extend(PTMAnt.Bridge,{
		...
		onMessage : function(evt){
			if((evt.data).search("Bridge Fine") > -1){
				PTMAnt.Version = evt.data.match(/\d*\.\d*\.\d*\.\d*/gi);
				
					if(PTMAnt.isDataReceivedFromBridge == false){
						if(PTMCheckVersion.check(PTMAnt.Version,[VIEWER_VERSION])){
...
PTMAnt.Extend(PTMAnt,{
	...
	runBridge : function(){
		location.href =  "printmade25://C|container||1000";
	},
...
var PMFRAME = "main";
// 공통 경로
var PMRETURNURL = location.protocol + '//' + location.hostname;
var PMLOADIMAGEPATH = location.protocol + '//' + location.hostname;
//var SERVER_PATH = location.protocol + '//' + location.hostname + '/ois/download/printmade2';
var SCRIPT_PATH = location.protocol + '//' + location.hostname + '/ois/js/printmade/scripts/';
//var FORM_PATH = location.protocol + '//' + location.hostname + '/ois/js/printmade/server_files/form.txt';
var FORM_PATH = '';
var SCRIPT_OPTION = 1;
//var SERVER_PATH = "http://localhost:9080/ois/js/printmade";
//var SERVER_PATH = location.protocol + '//' + 'www.nagosoft.co.kr' + '/setup';
...
// 설치 화면 이미지 경로
var INSTALL_IMG_URL = SERVER_PATH + '/ois/img/printmade3/imgs/';
...
var INSTALL_LINUX_MSG = '압축 파일을 해제 후, install.sh를 터미널에서 실행하십시오. 설치 시 브라우저를 종료하셔야 합니다.'; //빈칸으로 설정시 메시지가 안뜬다.

var UPDATE_ERROR_MSG = 'Printmade 3 업데이트 과정에서 오류가 생겼습니다. 새 버전을 설치합니다.';

/*Mac, Linux 브라우저 종료 알림*/
var INSTALL_MAC_INFO_MSG = 'Printmade 3 설치 시 브라우저가 종료됩니다';


var IFRAMECAPTURE = 1;
var PTMCOMPANYFORM = ''; // 2.0.1.11 로컬 form을 사용할 경우, 폴더명을 명기해준다. 20130729 
...
if ( (navigator.language || navigator.userLanguage).toLowerCase().indexOf('en') > -1 ) {
	...
	INSTALL_DIV_MSG = 'Web Print Solution. Printmade 3';
	INSTALL_LINUX_MSG = 'Uncompress an archive file and run install.sh in Terminal.';
	
	UPDATE_ERROR_MSG = 'Error in updating Printmade 3. Install latest version.';
	
	INSTALL_MAC_INFO_MSG = 'Printmade 3 Installing Browser Kill.';
}
...
var FULLCAPTURE = 0; // 전체캡쳐, 건들지마시오
var OFFPRINTERSETTING  = 1; //OFFPRINTERSETTING :  0 보임, 1 안보임

function PTMPrintWebPage( options )
{
	
	var license_list = ["192.168.224.207","192.168.224.206","203.235.68.78","203.235.68.99","203.235.68.101","opentest.ibk.co.kr"]

	for(i=0;i<license_list.length;i++){
	  if(location.href.match(license_list[i])){
	   LICENSE_CODE = "VVm4zvxbyNDKK+xVf0TDOKstYbIbB6J/KX7NRkfkfvDmidpFBObaU2GiVY2nGZk9";
	  }
	}
  ...
  // 아래의 options.frame_name에서 에러남.
	...
	if(location.port != ""){
		SERVER_PATH = location.protocol + '//' + location.hostname + ':' + location.port+ '/ois/download/printmade3';
		SCRIPT_PATH = location.protocol + '//' + location.hostname + ':' + location.port+ '/ois/js/printmade/scripts/';
		WIN_DOWNLOAD_URL = SERVER_PATH + '/download_files/Windows/Printmade3_setup.exe';
		MAC_DOWNLOAD_URL = SERVER_PATH + '/download_files/MacOSX/Printmade3forMac.dmg'
		UBUNTU_64BIT_URL = SERVER_PATH + '/download_files/Linux/Printmade3_Ubuntu_64bit.tar';
		UBUNTU_32BIT_URL = SERVER_PATH + '/download_files/Linux/Printmade3_Ubuntu_32bit.tar';
		FEDORA_32BIT_URL = SERVER_PATH + '/download_files/Linux/Printmade3_Fedora_32bit.tar';
		FEDORA_64BIT_URL = SERVER_PATH + '/download_files/Linux/Printmade3_Fedora_64bit.tar';
		FORMURL = "";
		INSTALL_IMG_URL = location.protocol + '//' + location.hostname + ":" + location.port+ '/ois/img/printmade3/imgs/';
		//FORM_PATH = location.protocol + '//' + location.hostname + ":" + location.port + '/ois/js/printmade/server_files/form.txt';
		FORM_PATH = '';
	}
  ...
  if ( options == undefined )
	...
	if( dmj('frameset',window.top.document)[0] != undefined && PMFRAME != ""){
		options.frame_name = PMFRAME;
	}
	...
	GPrintmade.option.SetIFrameCapture(IFRAMECAPTURE);
	//Cut
	...
	GPrintmade.option.SetOffPrintSetting( OFFPRINTERSETTING );	
...
//  dmj("body").append("<span style='position:absolute;height:65px;width:250px;top:"+ download_div_top +"px;left:"+download_div_left+";background-image:url("+INSTALL_IMG_URL+"load.png);z-index:10001'><\/span>");
    dmj(document.body).append("<div id='ptm-loading' ptm-loading-remove='true' style='position:fixed;z-index:10001;top:"+(document.body.clientHeight-300)/2+"px;left:"+(document.body.clientWidth-510)/2+"px'><img src='printmade_loading.gif' ptm-loading-remove='true'></div>");
...
function PTM_BeforePrinting( options, targetframe ) {
	...
	if(dmj("#printmade_chart").length > 0){
		options.objectcapture = 1;
	}else{
		options.objectcapture = 0;
	}
	...
	//인쇄영역 지정 확인
	//1. printmade_area 확인
	//2. printmade_area 영역이 존재하지 않으면  ibk_contents 확인
	//3. printmage_area 영역도 존재하지 않고 ibk_contents 영역도 없으면 전체인쇄 옵션으로 호출
	if(options.elementid == "#printmade_area"){
		SCRIPT_OPTION = 1;
		if(dmj("#printmade_area").length == 0){
			options.elementid = "#ibk_contents";
			SCRIPT_OPTION = 1;
			
			if(dmj("#ibk_contents").length == 0){
				options.elementid = "#ibkpop_contents";
				SCRIPT_OPTION = 1;
				if(dmj("#ibkpop_contents").length == 0){
		...
		if(dmj("#printmade_area").length > 0 && dmj("#printmade_area").height() == 0){
			options.elementid = "#ibk_contents";
			SCRIPT_OPTION = 1;
		...
		if(ispopupPrint() == false){
			options.formurl = FORM_PATH;
		}
	}

	if(dmj("#printmade_area").length > 0){
		var width = dmj("#printmade_area")[0].scrollWidth;
		if(width == 0){
			width = dmj("#printmade_area").width();
		}
		dmj("#printmade_area").attr("PTMPopupWidth",width);
	}
...
//		if(dmj(".subwrap").length > 0){
//			dmj(".subwrap").css('width',dmj(".subwrap").width());
//		}
					
//		if(dmj(".login").length > 0){
//			dmj(".login").css('height',dmj(".subwrap").height());
//		}
		FULLCAPTURE = 1;
		SCRIPT_OPTION = 1;
	}
	...
	if(dmj('#PTMLoadImage').length > 0) dmj('#PTMLoadImage').remove();
	
	if ( options.elementid != undefined ) {
	  
	 /* 
  	  if(dmj('.subtitle_wrap',targetframe.document).length >0){
  	    var ptmupimgsrc = dmj('.subtitle_wrap h3 img').attr('src');
  	    dmj('.subtitle_wrap',targetframe.document).append("<img id='ptm-title' src='"+ptmupimgsrc+"'>");
  	    dmj('.subtitle_wrap h3',targetframe.document).css('display','none');
  	    dmj('.subtitle_wrap .pt_btns',targetframe.document).css('display','none');
  	  }*/
  	  
  	  
  	//	PMPAGEWIDTH = dmj(options.elementid, targetframe.document)[0].scrollWidth;
  	//	dmj( options.elementid, targetframe.document ).width( dmj(options.elementid, targetframe.document)[0].scrollWidth );		
	}
}

function PTM_AfterPrinting( options, targetframe ) {
	if(dmj('noembed').length > 0) dmj('noembed').css('display','');
	FULLCAPTURE = 0;
	if(dmj('#ptm-loading').length > 0) dmj('#ptm-loading').remove();
	if(dmj('#ptm-loading-bg').length > 0) dmj('#ptm-loading-bg').remove();
	...
	if ( options.elementid != undefined ) {
		if(dmj('.subtitle_wrap',targetframe.document).length >0){
      	  dmj('#ptm-title').remove();
    	    dmj('.subtitle_wrap h3',targetframe.document).css('display','block');
    	    dmj('.subtitle_wrap h3',targetframe.document).css('float','left');
    	    dmj('.subtitle_wrap .pt_btns',targetframe.document).css('display','block');
    	}
	}*/
...
function PTM_DownloadDialog() {
	...
	if(clientHeight > maxScrollHeight && clientHeight > frame.document.body.scrollHeight){
	...
	if ( GPrintmade.isFirefox3 == true || GPrintmade.isFirefoxOver3 == true ) {
		now_focus = frame.document.body;
	} else {
			now_focus = frame.document.body;
//		now_focus = frame.event.srcElement;
//		now_focus = now_focus.parentElement;
	}
		

	dmj( now_focus ).append( '<div id = "ptm_down" align = center style="position:absolute;top:' + download_div_top + 'px;left:' + download_div_left + 'px;width:'+dialog_width+'px;height:'+dialog_height+'px;background-image:url(\'' + INSTALL_IMG_URL + 'setup.jpg\');z-index:5001;display:block"></div>' );
	dmj( now_focus ).append( '<div id = "ptm_bg" style = "filter:alpha(opacity=50);opacity:0.5;position:absolute;top:0;left:0;background-color:#000000;z-index:5000;display:block;width:' + frame.document.body.scrollWidth + 'px;height:' + maxScrollHeight + 'px"></div>' );
	...
	if (GPrintmade.isWindows == true) {
		...
		download_url = MAC_DOWNLOAD_URL;
		var msg = '';
		if('' != INSTALL_MAC_INFO_MSG)
		{
			msg = 'onClick="alert(\''+INSTALL_MAC_INFO_MSG+'\');"';
		}
		$down_div.append('<a href = "' + download_url + '" id = "ptm_download" '+msg+'><img alt = "다운로드" src = "' + INSTALL_IMG_URL + 'download.jpg" style = "border:0"></a>');
	...
	//2013-08-21 인스톨과정에서 BeforePrint 함수만 실행되어 버튼이 사라지는 오류를 수정했음.
...
var PTMOptionObject = Soo.Class.create({
	...
	SetOffPrintSetting : function( offprintsetting ) {
		this._offprintsetting = offprintsetting;
	},
	...
	SetIFrameCapture : function( iframecapture ) {
		this._iframecapture = iframecapture;	
	},
	GetIFrameCapture : function( iframecapture ) {
		return this._iframecapture;	
	},
	...
	GetParameterString : function() {
		...
		this._AddParameter('LICENSE', this._license); //20150713

		this._AddParameter('RENDERINGTYPE', this._renderingtype);

		this._AddParameter('FORMURL', this._formurl);

		this._AddParameter('ORIGINALURL', this._originalurl); //20150713
		...
		this._AddParameter('OFFPRINTERSETTING', this._offprintsetting);
...
//		this._frame = frame;
	...
	VpInstallForWin : function(){
		if(this._isIE == true){
			var vpJs = document.createElement("script");
			vpJs.src = "/ois/wizvera/veraport/veraport20.js";
			document.body.appendChild(vpJs);
			if(window.confirm("프린트메이드를 설치하시겠습니까?") == true){
				if(vp_checkVp20Install(false)){
					VP_axInstallSelect("Printmade");
					return false;
				} else {
					plugin = null;
					PTM_DownloadDialog();
	...
	VpUpdateForWin : function(){
		if(this._isIE == true){
			var vpJs = document.createElement("script");
			vpJs.src = "/ois/wizvera/veraport/veraport20.js";
			document.body.appendChild(vpJs);
			if(window.confirm("최신 프린트메이드를 설치하셔야 진행하실 수 있습니다.\n 업데이트를 진행하시겠습니까?") == true){
				if(vp_checkVp20Install(false)){
					vp_setKillBrowser('all,silent',PMRETURNURL);
					VP_axInstallSelect("Printmade");
					return false;
				} else {
					plugin = null;
...
var PTMCaptureController = Soo.Class.create({
	...
	CaptureWithElement : function ( id ) {
		if ( id == 'ptm_plugin' || id.indexOf('printmade')  != -1  || id == 'KeySharpBizPlugin') {// KeySharpBizPlugin와 충돌이 일어남. 이때 무시해야될듯
			return '';
		}
...
var PTMMainController = Soo.Class.create({
	...
	Run:function(target){
		...
		}else if(GPrintmade.isFirefoxOver3 == true || GPrintmade.isFirefox3 == true){
			if(!this.initNPAPI(target)) return;
		}
		this.target = target;
		this._install = null;
	...
	_FindReplaceTag : function() {
		...
		if(GPrintmade.option.GetIFrameCapture() == 1){
			this.isIframeTagExist = this._FindTag(this.target, 'iframe');
			result = this.isObjectTagExist || this.isEmbedTagExist || this.isMarqueeTagExist || this.isIframeTagExist;
			return result;
		}
...
/* 		this._insertID(framewin); */
		...
		//	프로그레스바 제거 (130930)
		result_body = result_body.replace(/<[^<]*?ptm-loading-remove\s*=[\"|\'](\S*)[\"|\'].*?>/gi,"");

		// no TouchEn key (131008)
		result_body = result_body.replace(/no TouchEn key/gi,"");
		...
		if(unique_id==1 && SCRIPT_OPTION == 1){
			result = result + '<head><script> var FULLCAPTURE = '+ FULLCAPTURE +'; </script>' + result_head;
			result += '<script type = \"text/javascript\" src=\"'+ SCRIPT_PATH +'ptm-selector.js\"></script>';
			result += '<script type = \"text/javascript\" src=\"'+ SCRIPT_PATH +'ptm-main.js\"></script></head>';		
		}else{
		...
		*/
		try{
			if(GPrintmade.isLinux){
				...
				//alert(this._appdatapath + '/Printmade3/temp/' + unique_id + '.html\0');
		...
		if (unique_id == 1)
		{
			...
			if(GPrintmade.isMac){
				...
				{
					var path = this._appdatapath + '/Printmade3/Viewer.exe';
					if(this._plugin.getRunPath()){
						path = this._plugin.getRunPath();
						path = path.replace(/\\/g, '/');
						path = path + '/Printmade3/Viewer.exe';
					}
					
					//alert(this._appdatapath);
					this._plugin.runViewer(path, '\"' + this._appdatapath + '/Printmade3/temp/1.html\" \"' + GPrintmade.option.GetParameterString() + '\"',PMVIEWERMODAL);
					//
					//this._plugin.runViewer(path, '\"PTM://1.html\" \"' + GPrintmade.option.GetParameterString() + '\"',PMVIEWERMODAL);       					
...
//	result = result + this._ResizeBody();
	...
	_ReplaceToCapturedImage : function(string, plugincapture) {
		...
		if(GPrintmade.option.GetIFrameCapture() == 1){
			 if (GPrintmade.isIframeTagExist == true) {
				var regex2 = /<iframe\s*([^\>])*?\sid\s*=\s*(\"|\')*\w*\2\s*(.|\n)*?>/gi
				result = result.replace(regex2, function(word) {
